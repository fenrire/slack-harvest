...
- `--all` 수집 중 한 채널의 `RuntimeError`(`channel_not_found` 등)가 fetch 전체를 중단시켜, slack 배치가 6/22~6/24 3일 연속 `exit=1`로 실패하던 문제 수정
- 원인: `#게임사업본부x기술개발본부-pm논의방`(`C09T95XTWQY`)가 삭제/접근불가가 됐는데, 한 채널의 예외가 루프 밖으로 전파돼 나머지 58채널 수집까지 전부 중단
- 채널 루프를 `try/except RuntimeError`로 격리: 채널 단위 에러는 경고 후 건너뛰고 계속, 종료 시 실패 채널 목록을 요약 출력 + ops.log에 `실패 N개` 기록. exit 0 유지 → 채널 삭제 등 정상적 라이프사이클 변화로 배치가 멈추지 않음
- 검증: 재실행 시 해당 채널만 건너뛰고 "수집 완료!"(exit 0) 확인
